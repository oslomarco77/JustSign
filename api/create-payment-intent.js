...
async function createBeamCharge(contractId, product) {
  ...
  const r = await fetch(`${BEAM_BASE}/api/v1/charges`, {
    ...
    body: JSON.stringify({
      amount: PRICE_SATANG,                 // สตางค์ (฿790 = 79000)
      currency: 'THB',
      paymentMethod: {
      ...
      returnUrl: product === 'sale' ? 'https://sale.signdee.com/'
              : product === 'nda' ? 'https://nda.signdee.com/'
              : product === 'emp' ? 'https://emp.signdee.com/'
              : 'https://app.signdee.com/',
    }),
  });
...
// ── action: dev_skip — ข้ามการชำระเงินสำหรับทดสอบ (เจ้าของระบบเท่านั้น) ──
//   ตรวจ secret กับ env DEV_SKIP_SECRET (ไม่ตั้ง env = ปิดการข้าม) → service_role เขียน payment
async function handleDevSkip(body, res) {
  const secret = process.env.DEV_SKIP_SECRET || '';
  if (!secret) return res.status(403).json({ ok: false, error: 'dev_skip_disabled' });
  if (!body.secret || body.secret !== secret) return res.status(401).json({ ok: false, error: 'bad_dev_secret' });
  const id = body.contract_id;
  if (!id) return res.status(400).json({ ok: false, error: 'contract_id required' });
  const table = body.kind === 'sale' ? 'sale_contracts' : 'contracts';
  const patch = table === 'contracts'
    ? { payment_completed: true, payment_ref: 'DEV_SKIP' }
    : { payment_completed: true };
  try {
    await sbPatch(table, { id }, patch);
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    /* ⚠️ ห้ามย้ายการตรวจ STRIPE_SECRET_KEY กลับมาไว้ก่อนจุดนี้
       เดิมตรวจไว้ตั้งแต่ต้น handler ทำให้คำขอที่ใช้ Beam (QR PromptPay)
       ถูกบล็อกไปด้วยทั้งที่ไม่ได้แตะ Stripe เลย → สร้าง QR ไม่ได้ทั้งระบบ
       ทั้ง rent และ sale ส่ง gateway:'beam' มา แต่ตายที่ด่านนี้ก่อนถึงโค้ด Beam
    ...
    // ── Stripe Connect (มัดจำ) — route ตาม action ก่อนถึง flow ค่าบริการเดิม ──
    ...
    // ถึงตรงนี้แปลว่าจะใช้ Stripe จริง จึงต้องมี key
    ...
    const params = toForm({
      ...
      confirm: 'true',
      receipt_email: email,
      metadata: { contract_id: contractId, product: (body.product === 'sale' ? 'SignDee sale-purchase contract' : body.product === 'nda' ? 'SignDee NDA contract' : body.product === 'emp' ? 'SignDee employment contract' : 'SignDee rental contract') },
    });
