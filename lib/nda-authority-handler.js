...
const Authority = require('../api/_nda_authority.js');
const { deliverWorkspaceAcceptance } = require('./workspace-delivery-client.js');
...
async function handler(req, res) {
  ...
  if (!['create_initial_version', 'sign', 'reserve_workspace_binding',
    ...
    'confirm_workspace_acceptance', 'deliver_workspace_acceptance'].includes(body.action)) {
    return reply(res, 400, { ok: false, code: 'unsupported_action' });
  }

  // SD-407C — outbound delivery to the Sign Dee receiver.
  //
  // Placed before the evidence group because it must NOT reach
  // persistSignedEvidence: this action performs no database write at all. It
  // takes two identifiers that are already durable and hands them to the
  // receiver, which then calls back into resolve_workspace_acceptance to
  // establish the facts for itself.
  //
  // Retry is caller-driven by design. Re-issuing the same request is safe —
  // the identity is stable and the receiver is idempotent on binding_id — but
  // nothing here retries automatically, so a transient fault can never fan out
  // into duplicate Workspace effects.
  if (body.action === 'deliver_workspace_acceptance') {
    ...
    let request;
    try { request = Authority.signedEvidenceRequest(body); }
    catch (error) {
      const candidate = String(error.message || '');
      return reply(res, 400, { ok: false, code: /^invalid_/.test(candidate) ? candidate : 'invalid_request' });
    }
    try {
      const result = await deliverWorkspaceAcceptance('nda', {
        binding_id: request.bindingId,
        signed_document_reference: request.signedDocumentReference,
      });
      return reply(res, 200, { ok: true, ...result });
    } catch (error) {
      // `error.code` is a fixed token from the delivery client. It never
      // contains a secret, a signature, the receiver URL, the evidence
      // reference, or any response body.
      ...
      return reply(res, retryable ? 502 : 409, { ok: false, code, retryable });
    }
  }
